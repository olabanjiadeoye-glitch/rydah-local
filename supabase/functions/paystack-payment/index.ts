...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.action === "initialize") {
      ...
      const jobResponse = await supabaseRequest(`jobs?id=eq.${encodeURIComponent(body.job_id)}&select=id,customer_id,provider_id,status,quote_status,quoted_amount,payment_status,contact_email&limit=1`);
      const jobs = await jobResponse.json() as Array<{ id: string; customer_id: string | null; provider_id: string | null; status: string; quote_status: string; quoted_amount: number | null; payment_status: string; contact_email: string | null }>;
      const job = jobs[0];
      ...
      const initialized = await paystack("transaction/initialize", {
        ...
        body: JSON.stringify({
          ...
          metadata: JSON.stringify({ job_id: job.id, customer_id: user.id, provider_id: job.provider_id }),
        }),
      });
      ...
      await supabaseRequest("payments", {
        ...
        body: JSON.stringify({ job_id: job.id, method: "paystack", reference, is_test: secretKey.startsWith("sk_test_") }),
      });

      return json({ authorization_url: initialized.data.authorization_url, access_code: initialized.data.access_code, reference });
    }

    if (body.action === "verify") {
      if (!body.reference) return json({ error: "reference is required" }, 400);
      const paymentResponse = await supabaseRequest(`payments?reference=eq.${encodeURIComponent(body.reference)}&customer_id=eq.${encodeURIComponent(user.id)}&select=*&limit=1`);
