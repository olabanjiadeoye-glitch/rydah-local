  ...
  async function detectCurrentLocation() {
    ...
    try {
      ...
      if (!nearest || nearest.distanceKm > 60) {
        ...
        setSort("Recommended");
        setGpsMessage("GPS detected outside Rydah's current Lagos, Abuja, Ibadan, Warri and Port Harcourt coverage. Use the area filter to browse providers in a target city.");
        return;
      }
      ...
      {selectedProvider && (
        <div className="fixed inset-0 z-50 flex items-end bg-black/80 p-4 sm:items-center sm:justify-center">
          <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-[#121212] p-6">
            ...
            <div className="mt-5 rounded-2xl border border-white/10 bg-[#0D0D0D] p-4 text-sm leading-6 text-zinc-400">
              <p className="font-black text-[#D4AF37]">TRUST SIGNALS</p>
              <p className="mt-1">Customer rating and completed-job totals above come from real Rydah activity. New providers are shown as new rather than being given invented ratings or testimonials.</p>
            </div>
